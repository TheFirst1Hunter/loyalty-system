...
import { StatisticIncome, StatisticOrder } from './entities/statistic.entity';
...
export class StatisticsService {
  ...
  async getOrders(filter: QueryStatisticsDto): Promise<StatisticOrder> {
    ...
    return { orderCount, orderCountWithCredit };
  }

  async getIncome(filter: QueryStatisticsDto): Promise<StatisticIncome> {
    // Destruct the from-to date string
    const { from, to } = filter;
